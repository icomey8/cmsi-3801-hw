...
powers :: Integer -> [Integer] --fix, use integral type class instead
...
meaningfulLineCount path = do
    ...
    return $ length $ filter meaningfulLine $ lines contents
        where
          meaningfulLine line = not (isSpace (head line)) && not ("--" `isPrefixOf` line)
...
instance (Show a) => Show (BST a) where
  ...
  -- still need to remove superflouous parantheses